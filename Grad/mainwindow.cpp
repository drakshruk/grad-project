...
void MainWindow::on_actiontest_triggered()
{
    ...
    int searchRange = 15;
    this->getDataDialog->setWindowTitle("Write the search raadius for the alignment");
    this->getDataDialog->setPlaceholderText("Search radius");
    if(getDataDialog->exec()){
        searchRange = int(getDataDialog->value);
    }

    double xGrad = 0., yGrad = 0.;
    xGrad = xGradMat[mPos.x()][mPos.y()];
    yGrad = yGradMat[mPos.x()][mPos.y()];
//        qDebug() << " xGrad: " << xGrad << " yGrad: " << yGrad;
    double gradMag = sqrt(xGrad*xGrad + yGrad*yGrad);
    if (gradMag > 1e-6) {
        xGrad /= gradMag;
...
//    std::pair<int,int> bounds2 = ImageProcessor::alignProfiles(referenceProfile, profile2.points, left1, right1, searchRange);
    QVector<QPointF> profileToAlign = profile2.points;


    //====================================================
    std::pair<int,int> bounds2 = {0,0};
    ...
    for(int i = 0; i < referenceProfile.size(); i++){
        median1 += referenceProfile[i].y();
    }
    median1 /= referenceProfile.size();

    for(int i = 0; i < referenceProfile.size(); i++){
        dispersion1 += (referenceProfile[i].y()-median1)*(referenceProfile[i].y()-median1);
    }
    dispersion1 = sqrt(dispersion1)/referenceProfile.size();

    for(int i = 0; i < referenceProfile.size(); i++){
        referenceProfile[i].setY((referenceProfile[i].y()-median1)/dispersion1);
    }
    int leftEdge = left1 - searchRange;
    leftEdge = leftEdge < 0 ? 0 : leftEdge;

    int rightEdge = right1 + searchRange;
    rightEdge = rightEdge >= referenceProfile.size() ? referenceProfile.size() - 1 : rightEdge;
    QVector<GraphWidget*> wids;
    for(int leftIncr = leftEdge; leftIncr <= left1; leftIncr++){
        for(int rightIncr = rightEdge; rightIncr >= right1; rightIncr--){

            QVector<QPointF> shiftedProfile = ImageProcessor::cutProfile(referenceProfile, leftIncr, rightIncr);
            shiftedProfile = ImageProcessor::reInterpolateProfile(shiftedProfile, referenceProfile.size());
            ...
            for(int j = 0; j < profileToAlign.size(); j++){
                x1.push_back(referenceProfile[j].x());
                y1.push_back(referenceProfile[j].y());
                x.push_back(profileToAlign[j].x());
                y.push_back(profileToAlign[j].y());
            }
//            GraphWidget* grwid = new GraphWidget();
//            grwid->plotTwoGraphs(x,y,x1,y1);
//            grwid->show();
            double tmp = ImageProcessor::calculateResidualOfProfiles(referenceProfile,shiftedProfile);
            if(tmp < minResidual) {
                bounds2 = {leftIncr,rightIncr};
                minResidual = tmp;
            }
        }
    }
...
//        qDebug() << " left2: " << left2 << " right2: " << right2;
//        qDebug() << " left shift: " << leftShift << " right shift: " << rightShift;
        double xx = (left1*rightShift - right1*leftShift)/(leftShift - rightShift);

//        double mu = nSig*dSigma*(-1. + 2.*xx/nSig/dSigma);
        ...
        int xNewOriginal = static_cast<int>(muS+0.5);
        int yNewOriginal = (rightShift-leftShift)/(static_cast<double>(right1-left1));

    int xNew = mPos.x() + shift*xGrad;
    int yNew = mPos.y() + shift*yGrad;

    qDebug() << " New x: " << xNew << " New y: " << yNew;
    qDebug() << " Old x: " << mPos.x() << " Old y: " << mPos.y();
        qDebug() << " new x original: " << xNewOriginal << " new y original: " << yNewOriginal;
    qDebug() << " residual: " << ImageProcessor::calculateResidualOfProfiles(
                    ImageProcessor::reInterpolateProfile(ImageProcessor::cutProfile(profile1.points,left1,right1),100),
                    ImageProcessor::reInterpolateProfile(ImageProcessor::cutProfile(profile2.points,left2,right2),100));
    tmp.setPixel(xNew, yNew, qRgb(255,0,0));
    tmp.setPixel(mPos.x(), mPos.y(), qRgb(0,0,255));
    whiteIm.setPixel(xNew, yNew, qRgb(255,0,0));
    whiteIm.setPixel(mPos.x(), mPos.y(), qRgb(0,0,255));
    im->setImage(tmp);
